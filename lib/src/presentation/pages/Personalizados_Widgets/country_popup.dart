...
import 'package:woin/src/widgets/city_popups.dart';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        color: Colors.white,
        child:Container(
          child: _searchResult.length == 0 || _searchController.text.isEmpty ? StreamBuilder<List<Country>>(
            stream: countryService.CountryList,
            builder: (context, snapshot) {
              return snapshot.hasData
              ? ListView.builder(
                itemCount: snapshot.data.length,
                itemBuilder: (context, index){
                  _countryList = snapshot.data;
                  var data = snapshot.data[index];
                  return ListTile(
                    onTap: (){
                      Provider.of<DoumentTypeProvider>(context, listen: false).getCountry = data ;
                      Navigator.of(context).pop();
                      countryService.obtenerCiudadPais(data.id);
                      showDialogCity(context);
                    },
                    leading:Image.network(data.flag,
