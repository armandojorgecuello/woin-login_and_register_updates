...
abstract class _CustomLayoutStateBase<T extends _SubSwiper> extends State<T>
  ...
  @mustCallSuper
  void afterRender() {
    RenderObject renderObject = context!=null ? context.findRenderObject():null;
    Size size = renderObject.paintBounds.size;
    _swiperWidth = ResponsiveFlutter.of(context).wp(100);
  ...
  void _onPanUpdate(DragUpdateDetails details) {
    print("AQUI" + _swiperWidth.toString());
    if (_lockScroll) return;
...
Offset _getOffsetValue(List<Offset> values, double animationValue, int index) {
  ...
  if (animationValue >= 0.5) {
    if (index < values.length - 1) {
      dx = dx + (values[index + 1].dx - dx) * (animationValue - 0.5) * 8.0;
      dy = dy + (values[index + 1].dy - dy) * (animationValue - 0.5) * 8.0;
    }
  } else {
    if (index != 0) {
      dx = dx - (dx - values[index - 1].dx) * (0.5 - animationValue) * 8.0;
      dy = dy - (dy - values[index - 1].dy) * (0.5 - animationValue) * 8.0;
    }
  }
...
class CustomLayoutOptionV2 {
  final List<TransformBuilder> builders = [];
  final int startIndex;
  final int stateCount;

  CustomLayoutOptionV2({this.stateCount, this.startIndex})
      : assert(startIndex != null, stateCount != null);

  CustomLayoutOptionV2 addOpacity(List<double> values) {
    builders.add(new OpacityTransformBuilder(values: values));
    return this;
  }

  CustomLayoutOptionV2 addTranslate(List<Offset> values) {
    builders.add(new TranslateTransformBuilder(values: values));
    return this;
  }

  CustomLayoutOptionV2 addScale(List<double> values, Alignment alignment) {
    builders
        .add(new ScaleTransformBuilder(values: values, alignment: alignment));
    return this;
  }

  CustomLayoutOptionV2 addRotate(List<double> values) {
    builders.add(new RotateTransformBuilder(values: values));
    return this;
  }
}

class _CustomLayoutSwiperV2 extends _SubSwiper {
  final CustomLayoutOptionV2 option;

  _CustomLayoutSwiperV2(
      {this.option,
      double itemWidth,
      bool loop,
      double itemHeight,
...
class _CustomLayoutState extends _CustomLayoutStateBase<_CustomLayoutSwiperV2> {
  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
  ...
  @override
  void didUpdateWidget(_CustomLayoutSwiperV2 oldWidget) {
    _startIndex = widget.option.startIndex;
    _animationCount = widget.option.stateCount;
    super.didUpdateWidget(oldWidget);
  }
